...
describe("Custom errors", () => {
 ...
 test("400: Responds with 'bad request' when uid contains invalid characters", () => {
  ...
  test("POST 400: Responds with 'bad request' if user id is missing (empty string)", () => {
    const attemptedReview = {
    review_id: 8,
    fruit: "Lime",
    body: "Comment",
  ...
  .expect(400)
  .then(({body}) => {
expect(body.msg).toBe("Bad request - user must be logged in to comment")
  })
  })
  ...
  test("POST 400: Responds with 'bad request' if review is missing a fruit", () => {
    ...
    .expect(400)
    .then(({body}) => {
      expect(body.msg).toBe("Bad request - fruit selection must be specified in order to leave a comment")
    })
  })
  test("POST 400: Responds with 'bad request' if a review is missing a rating", () => {
    ...
    .expect(400)
    .then(({body}) => {
      expect(body.msg).toBe("Bad request - rating must be added in order to post review")
    })
  })
})